...
| # | 問題 | 嚴重度 | 說明 |
|---|------|:------:|------|
| 4 | watchdog 開網路安全 | ~~HIGH~~ 已解決 | 實作用 `network_mode: none` + volume-based IPC（比 deploy-net 更安全）。不需要 deploy-net |
...
| # | 回覆 |
|---|------|
| 4 | ✅ 實作用 volume-based IPC（`network_mode: none`），比 deploy-net 更安全。不需要獨立網路 |
...
| 條件 | 狀態 | 性質 |
|------|:----:|:----:|
| AB-4 完成（PBO 修正） | ✅ Step 1-3 完成，Step 4 自動 | 已解決 |
| PaperDeployer API 驗證能跑 | ✅ 10 項測試全過（deploy/get_active/update_nav/kill/stop/reload） | 已解決 |
| 手動端到端流程跑通 3 次 | ✅ 2 次通過（factor→strategy→weights→deploy→NAV track） | 已解決 |
| 定義 paper trading 決策標準 | ✅ 已在 §11 定義 | 已解決 |
| Validator 職責定義 | ✅ watchdog 唯一驗證 | 已解決 |
| Phase AD 數據刷新 | ✅ Phase 1-3 完成（2026-03-31） | 非 blocking，已解決 |

**0 個 BLOCKING。AG 可在研究產出 L5 因子後啟動。**

### 2026-03-31 驗證備註

- Step 2「10 項測試」為手動 E2E 驗證，測試檔案不在 repo（非自動化回歸測試）
- Step 5 比較報告有 decision logic 但未接通 benchmark 實際 NAV — 需 L5 因子部署後補完
- Step 7 因子健康檢查 + 淘汰機制未實作，待部署後實作（門檻已定為 ICIR < 0.15）
- §4 deploy-net 未建立，但不需要 — watchdog 用 `network_mode: none` + volume-based IPC 更安全
