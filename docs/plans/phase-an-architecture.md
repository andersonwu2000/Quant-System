...
### AN-1：拆 app.py startup（P0）

**現狀**：`src/api/app.py:59` 的 lifespan 函式同時負責 app 建立、portfolio restore、execution 初始化、quote wiring、price polling、monitoring loop、kill-switch loop、router 掛載。

**目標**：抽成三層：
- `src/api/bootstrap/runtime.py` — app 建立、config 載入、lifespan 管理
- `src/api/bootstrap/market.py` — quote feed、price polling、ShioajiFeed wiring
- `src/api/bootstrap/monitoring.py` — monitoring loop、kill-switch、heartbeat

**風險**：Paper trading 正在跑，改啟動邏輯可能影響 server 穩定性。需要先寫整合測試。
...
| 檔案 | 行數 | 切法 | 拆後上限 |
|------|:----:|------|:--------:|
| evaluate.py | 2205 | gates/ + normalization/ + report/ | 維持單檔（READ ONLY 安全理由） |
| validator.py | 1722 | checks/statistical.py + checks/economic.py + checks/descriptive.py | 500 |
| jobs.py | 1117 | pipeline/refresh.py + pipeline/execute.py + pipeline/reconcile.py | 400 |
| auto_alpha.py | 1003 | routes/ + deploy/ + monitor/ | 400 |
| optimizer.py | 945 | methods/mean_variance.py + methods/risk_parity.py + methods/black_litterman.py | 400 |

**驗收方式**：拆分後 public API 不變（import path 相容）。每個新模組需有對應測試。
