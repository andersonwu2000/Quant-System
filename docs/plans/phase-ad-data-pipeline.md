...
> 狀態：🚧 Phase 1-3e 實作完成（含來源分離），Phase 4 待做，FinMind 數據重下中
...
| 1a-2 | Quality Gate（fail-closed，交易前閘門） | `src/data/quality_gate.py` | ✅ |
| 1a-3 | 排程整合（refresh → gate → pipeline） | 修改 `src/scheduler/jobs.py` | ✅ |
| 1a-4 | 測試 | `tests/unit/test_data_refresh.py`, `test_quality_gate.py` | ✅ 33 tests |

### Phase 1b：研究基礎（P1，下週） ✅

| 步驟 | 內容 | 新增/修改檔案 | 狀態 |
|------|------|-------------|------|
| 1b-1 | Securities Master（SQLite，非 parquet） | `src/data/master.py` + `quant.db` | ✅ |
| 1b-2 | Registry 定義（數據集 metadata） | `src/data/registry.py` | ✅ |
| 1b-3 | TWSE/TPEX Provider（僅 OpenAPI 日增量） | `src/data/sources/twse.py` | ✅ |
| 1b-4 | CLI 工具（status + refresh） | `src/data/cli.py` | ✅ |

### Phase 2：品質強化（P1，兩週內） ✅

| 步驟 | 內容 | 新增/修改檔案 | 狀態 |
|------|------|-------------|------|
| 2a | Schema 驗證（pandas assert） | `src/data/schemas.py` | ✅ |
| 2b | PIT 標注（財報公告日） | `src/data/schemas.py` | ✅ |
| 2c | Parquet metadata 血緣 | `src/data/refresh.py` | ✅ |
| 2d | Manifest CLI（即時掃描） | `src/data/cli.py` | ✅ |

### Phase 3：數據消費統一（P2，漸進遷移） ✅

| 步驟 | 內容 | 新增/修改檔案 | 狀態 |
|------|------|-------------|------|
| 3a | DataCatalog 統一存取層 | `src/data/data_catalog.py` | ✅ |
| 3b | evaluate.py `_load_all_data()` 改用 DataCatalog | `scripts/autoresearch/evaluate.py` | ✅ |
| 3c | 回測引擎切換（CatalogFeed） | `src/data/sources/catalog_feed.py` | ✅ |
| 3d | Paper trading 切換 | `src/data/sources/__init__.py` | ✅ |

> **遷移完成**：DataCatalog 按來源優先級讀取，CatalogFeed 作為 DataFeed 實作供回測/paper trading 使用（`data_source="catalog"`）。
...
| 步驟 | 內容 | 修改檔案 | 狀態 |
|------|------|---------|------|
| 3e-1 | Registry 加入 `source_dirs` 優先級 | `src/data/registry.py` | ✅ |
| 3e-2 | DataCatalog 按來源優先級查找 | `src/data/data_catalog.py` | ✅ |
| 3e-3 | refresh.py 寫入 `data/{source}/` | `src/data/refresh.py` | ✅ |
| 3e-4 | download_finmind_data.py → `data/finmind/` | `scripts/download_finmind_data.py` | ✅ |
| 3e-5 | quality_gate.py 透過 registry 讀 | `src/data/quality_gate.py` | ✅ |
| 3e-6 | CatalogFeed 跟隨 DataCatalog | `src/data/sources/catalog_feed.py` | ✅ |
| 3e-7 | 遷移 + 清除舊目錄 + 全量重下 Yahoo price | `scripts/migrate_data_dirs.py`, `scripts/download_yahoo_price.py` | ✅ |
| 3e-8 | 退役 `parquet_cache.py` | `src/data/sources/parquet_cache.py` | ⏳ 待做 |
| 3e-9 | evaluate.py DataCatalog base_dir | `scripts/autoresearch/evaluate.py` | ✅ 已在 3b 完成 |
| 3e-10 | 全量回歸測試 | 1,659 passed | ✅ |
