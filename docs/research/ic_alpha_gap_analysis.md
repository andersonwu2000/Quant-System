...
兩個維度：**profitability**（選股是否跑贏大盤）和 **novelty**（和現有因子庫的差異）。

ICIR 不需要作為目標 — L2 gate 已強制（≥ 0.30）。ICIR 是門檻不是目標；有 profitability 的因子幾乎一定有 ICIR，反之不然（當前 110 個 L5 因子的教訓）。

| 信號 | Agent 看到 | 防過擬合 |
|------|-----------|---------|
| L5b profitability（top quintile > universe） | pass/fail only | 不洩漏量級 |
...
代碼 gate 比文字引導可靠。program.md 提供**事實**（可用數據、評估維度），不規定方向。
