...
|------|:----:|------|
| AB-4 完成 | ✅ | PBO 修正完成 |
| PaperDeployer 驗證 | ✅ | 10 項單元測試 + 2 次手動 E2E 通過 |
| 手動端到端 3 次 | ✅ | 已完成 2 次（開盤後需補第 3 次） |
| 決策標準定義 | ✅ | §11 已定義 30天→90天 |
| Validator 職責定義 | ✅ | watchdog 唯一驗證 |

**AG 已基本就緒。** PaperDeployer 已驗證通過，等 L5 因子產出後即可啟動部署流程。開盤後需補第 3 次手動 E2E。
...
### 問題 4（MEDIUM → ✅ 已有）：月度再平衡 no-trade zone

~~`revenue_momentum.py` 目前沒有 no-trade zone。~~

**已實作**：`revenue_momentum.py:300-321` 有非對稱 no-trade zone（買入門檻 1.5%、賣出門檻 3%）。Phase AA 曾報告 +1.3% CAGR 改善，但那數字用舊代碼跑的（STALE_CONCLUSIONS_AUDIT 已標記不可信）。效果需用修正後代碼重新驗證，但功能本身已存在。

### 問題 5（LOW → ✅ 已修）：_revenue_cache TTL

~~`_revenue_cache` 永不過期。~~

**已修復**：`revenue_momentum.py:37` 加入 6 小時 TTL（`_REVENUE_CACHE_TTL = 3600 * 6`），`_preload_revenue()` 每次呼叫時檢查過期（L47）。Paper trading 連續運行不再有過時風險。
...
| 6a | Validator report 標註實際 IS 期間 | `validator.py` validate() 在 report 中記錄截斷後的 start/end | ~5 行 | ✅ 已修 |
| 6b | Validator report 標註 universe 大小 | 已有 universe_size check，確認和實際一致 | 0 行 | ✅ 已有 |

#### #6 CAGR 不一致排查結論（✅ 已查明 — 非 bug）

**現象**：BacktestEngine 直接跑 revenue_momentum → CAGR 2.21%。Validator 跑同策略 → CAGR 12.83%。差距 5.8 倍。

**三層原因疊加**：

| 原因 | 機制 | 影響 |
|------|------|------|
| **(a) V8 期間截斷（最大）** | Validator 截斷 `end` 到 `oos_start - 1`（~2024-09-24），排除 2025 虧損期。`validator.py:225-231` | IS-only CAGR 12.83% vs 含 2025 的全期間 2.21% |
| **(b) Universe 大小** | Validator 用 884-1067 支 vs 獨立回測 200 支，不同選股池產生不同持倉 | 中等影響 |
| **(c) BacktestConfig 差異** | Validator 顯式設定 `risk_rules`、`market_lot_sizes`（`.TW: 1000`），和 engine 預設不同 | 較小影響 |

**修復措施**：
- #6a ✅ 已修 — `ValidationReport` 加入 `actual_is_start` / `actual_is_end`，截斷後的實際期間對外可見
- #6b ✅ 已有 — universe_size 已在 report 中記錄

**結論**：不是 bug，是三個合理設計差異疊加。V8 截斷是正確的（防 IS/OOS 洩漏），但以前靜默截斷導致外部誤判。現在已透明化。

#### 更深層問題：主策略 OOS 表現差

12.83% IS CAGR 掩蓋了真正的問題：

- **vs_ew_universe = -4.59%** — 跑輸等權大盤，沒有選股 alpha
- **OOS Sharpe = -1.355** — 樣本外嚴重虧損
- 全期間 CAGR 只有 +4.37%（舊 +10.8% 是用有 bug 的代碼跑的）

**這表示 IS 的 12.83% 很可能是過擬合**。revenue_momentum 的 5 項篩選條件在樣本內有效，但 OOS 失效。

**這是研究問題，不是工程問題** — autoresearch 正在用 Method D（median |ICIR| ≥ 0.30）產出新因子，通過 L5 的因子經 Validator 驗證後會由 AG 部署流程替換主策略。工程層面的修復（#6a 透明化、equal-weight、regime hedge 關閉）已全部完成。
