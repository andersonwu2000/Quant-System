...
| A-2 | AA §441 | 無實盤 vs 回測滑價追蹤（標「P2」） | 微額實盤第一天就需要比較 fill price vs signal price | ~15 行 | ✅ 2026-03-29 — trade log 記錄 signal_price / fill_price / shortfall_bps / commission，每次 pipeline 自動計算 avg_shortfall |
...
**所有 YES 項目已完成。** 剩餘前置條件：paper trading 跑 1 週 + 通知設定。
