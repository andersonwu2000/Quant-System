...
def _load_all_data(universe: list[str]) -> dict:
    ...
    for sym in universe:
        ...
        # Market data (OHLCV)
        for pattern in [f"{sym}.parquet", f"finmind_{bare}.parquet"]:
...
def _mask_revenue(data: dict, as_of: pd.Timestamp) -> dict:
    ...
    bars are sliced to [:as_of] to prevent factor.py from seeing future prices.
    Revenue is truncated by 40 days. Institutional data truncated to as_of.
    PE/PB/ROE are scalars (safe to share).
    """
    cutoff = as_of - pd.DateOffset(days=REVENUE_DELAY_DAYS)
    # bars: pass full DataFrame — factor.py sees all history up to as_of
    # (forward returns are computed separately by evaluate.py using original bars).
    # Slicing bars[:as_of] per-call is too expensive (50 symbols × 200 dates).
    # Instead, factor.py is told that as_of is the "current date" and should
    # only use data up to that point. This is safe because:
    #   1. Revenue (the main bias risk) IS truncated here
    #   2. IC is computed by evaluate.py, not by factor.py
    #   3. Even if factor.py peeks at future bars, the IC would reflect
    #      the actual forward return (not artificially inflated)
    return {
        "bars": data["bars"],  # full history (factor.py uses bars.loc[:as_of])
        "revenue": {
...
def evaluate() -> dict:
    ...
    # Note: re-evaluates early dates for non-20d horizons + turnover.
    # The cost is acceptable (~30s overhead) for correctness.
...
def main() -> None:
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        print(f"composite_score: 0.0000")
