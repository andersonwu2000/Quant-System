...
│  │ Universe  │  │  Research  │  │  Decision  │  │   Execution     │  │
│  │ Selection │  │  & Scoring │  │  & Weights │  │   & Feedback    │  │
│  └──────────┘  └───────────┘  └───────────┘  └────────────────┘  │
│       ↑                                              │             │
│       └──────────── 績效回饋循環 ←────────────────────┘             │
│                                                                    │
...
class AutoAlphaConfig:
    ...
    # Decision
    min_icir: float = 0.3              # 最低 ICIR 門檻
    min_hit_rate: float = 0.52         # 最低 IC 勝率
    max_cost_drag: float = 200.0       # 最大成本拖累 (bps/年)
    use_rolling_ic: bool = True        # 使用 Rolling IC 動態加權
    regime_aware: bool = True          # 啟用 Regime 條件調整
...
Layer 4: 系統層
  ...
  ├─ 連續虧損告警 (5 天 → 通知)
  ├─ 券商斷線自動暫停
  └─ 研究結果異常偵測 (universe 過小、IC 全為 0)
```

### 5.2 緊急停止機制

```python
async def safety_check(config: AutoAlphaConfig) -> list[AlphaAlert]:
    alerts = []

    # 累計回撤
    dd = compute_max_drawdown_since_start()
    if dd >= config.emergency_stop_drawdown:
        alerts.append(AlphaAlert(level="critical", category="drawdown", ...))
        await pause_auto_alpha()  # 暫停自動交易
        await cancel_all_pending_orders()
        await notify("EMERGENCY: Auto-alpha paused due to drawdown")

    # 連續虧損
    consecutive = count_consecutive_loss_days()
    if consecutive >= config.max_consecutive_losses:
        alerts.append(AlphaAlert(level="warning", category="execution", ...))
        await notify(f"WARNING: {consecutive} consecutive loss days")
...
| `/api/v1/auto-alpha/config` | PUT | 更新配置 |
| `/api/v1/auto-alpha/start` | POST | 啟動自動化 |
| `/api/v1/auto-alpha/stop` | POST | 暫停自動化 |
| `/api/v1/auto-alpha/status` | GET | 當前狀態 (running/paused/stopped) |
| `/api/v1/auto-alpha/history` | GET | 歷史研究快照列表 |
| `/api/v1/auto-alpha/history/{date}` | GET | 指定日期的研究快照 |
| `/api/v1/auto-alpha/performance` | GET | 累計績效摘要 |
| `/api/v1/auto-alpha/alerts` | GET | 告警記錄 |
| `/api/v1/auto-alpha/run-now` | POST | 立即執行一次（測試用） |
...
```
src/alpha/auto/
├── config.py          ← AutoAlphaConfig + DecisionConfig
├── universe.py        ← UniverseSelector (Scanner + UniverseFilter)
├── researcher.py      ← AlphaResearcher (wraps AlphaPipeline)
├── decision.py        ← AlphaDecisionEngine (篩選 + Regime + Rolling IC)
├── executor.py        ← AlphaExecutor (下單 + 績效追蹤)
├── scheduler.py       ← AlphaScheduler (APScheduler job 定義)
├── store.py           ← AlphaStore (DB 持久化: snapshots + alerts + performance)
├── alerts.py          ← AlertManager (告警規則 + 通知)
├── safety.py          ← SafetyChecker (回撤熔斷 + 連續虧損)
└── __init__.py

依賴關係：
  config.py       → (無外部依賴)
  universe.py     → src/data/scanner.py, src/alpha/universe.py
  researcher.py   → src/alpha/pipeline.py, src/alpha/regime.py
  decision.py     → researcher.py, config.py
  executor.py     → src/execution/execution_service.py, src/risk/engine.py
  scheduler.py    → universe.py, researcher.py, decision.py, executor.py
  store.py        → src/data/store.py (DB)
