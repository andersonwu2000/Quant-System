"""Factor Research API routes — submit, run, status, history, alerts.

Split from auto_alpha.py (AP-5): research-related endpoints live here,
production alpha engine endpoints stay in auto_alpha.py.

Backward compat: auto_alpha.py re-exports these endpoints under the same
/auto-alpha prefix, so existing URLs keep working.
"""
...
class SubmitFactorRequest(BaseModel):
    """autoresearch session submit a factor that passed L4."""
...
async def submit_factor(
    ...
    """autoresearch session submit factor -> run Validator 15 checks -> auto deploy.

    Flow:
    1. Save factor.py to src/strategy/factors/research/{name}.py
    2. Wrap into Strategy via strategy_builder
    3. Run StrategyValidator 15 checks
    4. Pass threshold -> deploy to paper trading
    """
    ...
    # 0. Safety check — reject obviously malicious code
    FORBIDDEN_PATTERNS = [
    ...
    clean_name = _re.sub(r'^[0-9]+', '', clean_name)
    ...
    # 4. Deploy decision — hard/soft threshold
    deployed = False
...
@router.websocket("/ws")
async def factor_research_ws(
    websocket: WebSocket,
    token: str | None = Query(default=None),
) -> None:
    """WebSocket endpoint for real-time factor research pipeline events."""
