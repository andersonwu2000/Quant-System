"""Auto-generated research factor: rev_slope_6m
...
def compute_rev_slope_6m(symbols: list[str], as_of: pd.Timestamp) -> dict[str, float]:
    """Compute rev_slope_6m for all symbols at as_of date."""
    results = {}
    ...
    for sym in symbols:
        try:
            ...
            if len(revenues) < 6:
                continue
            x = np.arange(6)
            coeffs = np.polyfit(x, revenues[-6:], 1)
